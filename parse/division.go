...
func (m *Meta) DivOperands(left, right token.Value) (token.Value, error) {
	...
	if leftType == rightType {
		...
		case token.StringType:
			// TODO: this could mean take out any instance the right string within the left string
		...
		case token.BoolType:
			// TODO: not sure if this is correct, talk to Matt
		...
		case token.VarType:
			...
			var err error
			valueToken, err = m.DivOperands(left, right)
			if err != nil {
		...
		case token.ObjectType:
			...
			for key, value1 := range left.True.(map[string]token.Value) {
				if value2, ok := result[key]; ok {
					resultValue, err := m.DivOperands(value1, value2)
	...
	err := errors.New("Could not perform DivOperands on operands")
