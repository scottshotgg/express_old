	...
	case token.Assign:
		...
		case "assign":
			...
			if m.DeclaredType == "" {
				...
				return token.Value{}, errors.New("Could not find variable")
			} else if m.DeclaredType == token.SetType {
...
func (m *Meta) GetStatement() (token.Value, error) {
	...
	case token.Block:
		...
		m.Shift()
		block, err := m.CheckBlockWithNewMeta()
		if err != nil {
...
// CheckBlock ...
func (m *Meta) CheckBlockWithNewMeta() (token.Value, error) {
	fmt.Println("CheckBlockWithNewMeta", m)
	...
	for {
		...
		meta.DeclaredName = ""
		meta.DeclaredType = ""
		meta.DeclaredAccessType = ""
		meta.DeclaredActingType = ""
		meta.DeclaredValue = token.Value{}
